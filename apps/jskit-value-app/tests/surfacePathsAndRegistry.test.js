...
test("surface path helpers normalize paths and resolve surface by prefix", () => {
  ...
  assert.equal(matchesPathPrefix("/api/v1/console/members", "/api/v1/console"), true);
  assert.equal(matchesPathPrefix("/api/v1/consolex", "/api/v1/console"), false);
  assert.equal(resolveSurfaceFromApiPathname("/api/v1/console/members"), "console");
  assert.equal(resolveSurfaceFromApiPathname("/api/v1/admin/w/acme"), "admin");
  assert.equal(resolveSurfaceFromApiPathname("/api/v1/workspace/acme"), "app");
