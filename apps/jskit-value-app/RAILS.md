...
1. Before any coding or substantive response/action: read `LLM_CHECKLIST.md` and require its BOOTSTRAP gate to be satisfied.
2. If the BOOTSTRAP gate is not satisfied, respond only with `BLOCKED: bootstrap incomplete` and stop.
3. Before coding: identify affected surface(s), module seam(s), policy boundaries, and required tests.
4. During coding: preserve seam contracts and avoid cross-boundary leakage.
5. After coding: run checks, fix regressions, and document contract-impacting changes.
6. Never silently change auth/workspace/billing/realtime contracts.
