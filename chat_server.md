...
### Idempotency hash-versioning / canonicalization-compatibility hardening

- Fixed a forward-compatibility gap in the new immutable idempotency fingerprint plan: storing only `idempotency_payload_sha256` is not enough if canonicalization/normalization rules evolve in a future deploy.
- Added `idempotency_payload_version` to `chat_messages` and clarified the hash must be interpreted alongside an explicit canonicalization-version contract.
- Updated `sendMessage` replay/conflict logic to use version-aware hash comparison (and documented fallback behavior for legacy rows or unsupported historical versions) plus added test coverage expectations for version drift compatibility.
...
- `idempotency_payload_version` SMALLINT UNSIGNED NULL
  - version of the canonicalization/hash contract used to compute `idempotency_payload_sha256`
  - nullable when `idempotency_payload_sha256` is null (system/generated/backfill rows)
  - when hash is present, version must also be present; compare hash values only within the same version contract
...
5. Idempotency check:
   ...
   - compute `incomingIdempotencyPayloadVersion` (explicit canonicalization/hash contract version, e.g. `1`)
   - compute `incomingIdempotencyPayloadSha256` from the canonicalized logical send payload under that version contract (SHA-256 hex)
...
7. Insert `chat_messages` row
   - persist `idempotency_payload_version = incomingIdempotencyPayloadVersion` and `idempotency_payload_sha256 = incomingIdempotencyPayloadSha256` for user-sent rows with `clientMessageId`
   ...
   - apply the same idempotency payload-match check on the re-read result before deciding replay-success vs conflict (prefer stored immutable hash+version if present)
