<template>
  <section class="god-members-view py-2 py-md-4">
    <v-row>
      <v-col cols="12" lg="5">
        <v-card rounded="lg" elevation="1" border>
          <v-card-item>
            <v-card-title class="text-subtitle-1">Invite god members</v-card-title>
          ...
          <v-card-text>
            <v-alert v-if="!canInviteMembers" type="info" variant="tonal" class="mb-3">
              You do not have permission to send god invites.
            </v-alert>
      ...
      <v-col cols="12" lg="7">
        <v-card rounded="lg" elevation="1" border>
          <v-card-item>
            <v-card-title class="text-subtitle-1">God members and invites</v-card-title>
            <v-card-subtitle>Manage global god-surface roles.</v-card-subtitle>
          </v-card-item>
          <v-divider />
          <v-card-text>
            ...
            <v-alert v-if="!canViewMembers" type="info" variant="tonal" class="mb-0">
              You do not have permission to view god members.
            </v-alert>
                ...
                <v-list-item v-for="member in members" :key="member.userId" class="px-0">
                  <template #title>
                    <div class="d-flex align-center ga-2">
                      ...
                      <v-chip v-if="member.isGod" size="x-small" label color="secondary">God</v-chip>
                    </div>
                  </template>
                  <template #subtitle>
                    {{ member.email }}
                  ...
                  <template #append>
                    <v-select
                      ...
                      :disabled="!canManageMembers || member.isGod"
...
import { useGodMembersView } from "./useGodMembersView.js";

const { forms, options, collections, permissions, feedback, status, actions } = useGodMembersView();
