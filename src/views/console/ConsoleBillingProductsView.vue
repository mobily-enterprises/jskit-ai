<template>
  <section class="console-billing-products-view py-2 py-md-4">
    <v-card rounded="lg" elevation="1" border>
      <v-card-title class="d-flex flex-wrap align-center ga-3">
        <span class="text-subtitle-1 font-weight-bold">Billing products</span>
        <v-spacer />
        <v-btn variant="outlined" :loading="state.loading" @click="actions.refresh">Refresh</v-btn>
        <v-btn color="primary" @click="actions.openCreateDialog">Add product</v-btn>
      </v-card-title>
      <v-divider />
      <v-card-text>
        <v-alert v-if="state.productsLoadError" type="error" variant="tonal" class="mb-3">
        ...
        <div class="billing-products-table-wrap">
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-if="!state.tableRows.length">
                <td colspan="6" class="text-center text-medium-emphasis py-6">No billing products yet.</td>
              </tr>
    ...
    <v-dialog v-model="state.createDialogOpen" max-width="920">
      <v-card>
        ...
        <v-card-text>
          <v-row dense>
            ...
            <v-col cols="12" md="4">
              <v-select
                v-model="state.createForm.productKind"
                :items="meta.productKindOptions"
                item-title="title"
                item-value="value"
                label="Product kind"
                variant="outlined"
                density="compact"
                :error-messages="state.createFieldErrors.productKind"
              />
            </v-col>
            <v-col cols="12">
              <v-textarea
          ...
          <div class="text-body-2 font-weight-medium mb-1">Product price</div>
          <div class="text-caption text-medium-emphasis mb-2">{{ state.ui.priceDescription }}</div>
          ...
          <v-row dense>
            <v-col cols="12">
              <v-select
                ...
                persistent-hint
                :error-messages="state.createFieldErrors['price.providerPriceId']"
              />
            </v-col>
          </v-row>
        ...
        <v-card-actions class="justify-end">
          ...
          <v-btn color="primary" :loading="state.isSavingCreate" @click="actions.submitCreateProduct">
            Create product
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="state.viewDialogOpen" max-width="920">
      <v-card>
        <v-card-title class="text-subtitle-1 font-weight-bold">Product details</v-card-title>
        <v-divider />
        <v-card-text v-if="state.selectedProduct">
          <div class="product-details-header mb-4">
            <div>
              <div class="text-subtitle-1 font-weight-bold">
                {{ state.selectedProduct.name || state.selectedProduct.code || "Product" }}
              </div>
              <div class="text-caption text-medium-emphasis mt-1">
                Code: <code>{{ state.selectedProduct.code }}</code>
              </div>
            </div>
            <v-chip
              size="small"
              label
              :color="state.selectedProduct.isActive ? 'primary' : 'default'"
              :variant="state.selectedProduct.isActive ? 'tonal' : 'outlined'"
            >
              {{ state.selectedProduct.isActive ? "Active" : "Inactive" }}
            </v-chip>
          </div>
          ...
          <v-row dense>
            <v-col cols="12" md="7">
              <v-card rounded="lg" variant="tonal" class="h-100">
                <v-card-item>
                  <v-card-title class="product-section-title">Product identity</v-card-title>
                </v-card-item>
                <v-card-text>
                  <dl class="kv-grid">
                    <div class="kv-item">
                      <dt>Name</dt>
                      <dd>{{ state.selectedProduct.name || "-" }}</dd>
                    </div>
                    <div class="kv-item">
                      <dt>Code</dt>
                    ...
                    <div class="kv-item">
                      <dt>Status</dt>
                      <dd>{{ state.selectedProduct.isActive ? "Active" : "Inactive" }}</dd>
                    </div>
                  </dl>

                  <div class="mt-3">
                    <div class="text-caption text-medium-emphasis mb-1">Description</div>
                    <div class="description-block">{{ state.selectedProduct.description || "-" }}</div>
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
            <v-col cols="12" md="5">
              <v-card rounded="lg" variant="tonal" class="h-100">
                <v-card-item>
                  <v-card-title class="product-section-title">Price mapping</v-card-title>
                </v-card-item>
                <v-card-text>
                  <dl v-if="state.selectedProductPriceInfo" class="kv-grid">
                    ...
                    <div class="kv-item">
                      <dt>Product</dt>
                      <dd>{{ state.selectedProductPriceInfo.productName || "-" }}</dd>
                    </div>
                    <div class="kv-item">
                      <dt>Product ID</dt>
    ...
    <v-dialog v-model="state.editDialogOpen" max-width="920">
      <v-card>
        <v-card-title class="text-subtitle-1 font-weight-bold">Edit billing product</v-card-title>
        <v-divider />
        <v-card-text>
          ...
          <v-row dense>
            ...
            <v-col cols="12" md="4">
              <v-select
                v-model="state.editForm.productKind"
                :items="meta.productKindOptions"
                item-title="title"
                item-value="value"
                label="Product kind"
                variant="outlined"
                density="compact"
                :error-messages="state.editFieldErrors.productKind"
              />
            </v-col>
            <v-col cols="12">
              <v-textarea
            ...
            <v-col cols="12">
              <v-select
                ...
                :hint="state.ui.catalogPriceHint"
                persistent-hint
                :error-messages="state.editFieldErrors['price.providerPriceId']"
              />
            </v-col>
          </v-row>
        ...
        <v-card-actions class="justify-end">
          ...
          <v-btn color="primary" :loading="state.editSaving" @click="actions.saveEditedProduct">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </section>
