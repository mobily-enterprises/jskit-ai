...
function createRepository(dbClient) {
  ...
  const actors = {
    ...
    async upsert(workspaceId, payload = {}, options = {}) {
      ...
      const dbPatch = {
        ...
        last_fetched_at: payload.lastFetchedAt ? toMysqlDateTimeUtc(new Date(payload.lastFetchedAt)) : null,
  ...
  const actorKeys = {
    ...
    async upsert(workspaceId, actorId, payload = {}, options = {}) {
      ...
      const dbPatch = {
        ...
        rotated_at: payload.rotatedAt ? toMysqlDateTimeUtc(new Date(payload.rotatedAt)) : null,
        created_at: now
      };
  ...
  const posts = {
    ...
    async create(workspaceId, payload = {}, options = {}) {
      ...
      const now = toMysqlDateTimeUtc(nowDate);
      ...
      const dbRow = {
        ...
        published_at: payload.publishedAt ? toMysqlDateTimeUtc(new Date(payload.publishedAt)) : now,
    ...
    async update(workspaceId, postId, patch = {}, options = {}) {
      ...
      if (Object.hasOwn(patch, "editedAt")) {
        dbPatch.edited_at = patch.editedAt ? toMysqlDateTimeUtc(new Date(patch.editedAt)) : now;
      }
  ...
  const follows = {
    ...
    async createOrUpdate(workspaceId, payload = {}, options = {}) {
      ...
      const dbPatch = {
        ...
        accepted_at: payload.acceptedAt ? toMysqlDateTimeUtc(new Date(payload.acceptedAt)) : null,
        rejected_at: payload.rejectedAt ? toMysqlDateTimeUtc(new Date(payload.rejectedAt)) : null,
        undone_at: payload.undoneAt ? toMysqlDateTimeUtc(new Date(payload.undoneAt)) : null,
        updated_at: now
      };
  ...
  const notifications = {
    async create(workspaceId, payload = {}, options = {}) {
      ...
      const [id] = await client("social_notifications").insert({
        ...
        created_at: now,
        read_at: payload.readAt ? toMysqlDateTimeUtc(new Date(payload.readAt)) : null
      });
      ...
      await client("social_inbox_events")
        ...
        .update({
          ...
          processed_at: payload.processedAt
            ? toMysqlDateTimeUtc(new Date(payload.processedAt))
            : toMysqlDateTimeUtc(new Date()),
  ...
  const outboxDeliveries = {
    ...
    async markRetry(workspaceId, deliveryId, payload = {}, options = {}) {
      const client = resolveClient(dbClient, options);
      const now = toMysqlDateTimeUtc(new Date());
      const nextAttemptAt = payload.nextAttemptAt ? toMysqlDateTimeUtc(new Date(payload.nextAttemptAt)) : now;
      await client("social_outbox_deliveries")
        .where({
          workspace_id: parsePositiveInteger(workspaceId),
          id: parsePositiveInteger(deliveryId)
