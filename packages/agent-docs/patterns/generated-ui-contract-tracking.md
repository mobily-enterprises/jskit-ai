...
- CRUD filters are client-side by default: generated list pages create a page-local `listFilters.js`, wire `filterRuntime.queryParams` into `useCrudList(...)`, and render `CrudListFilterSurface`. Server filter behavior remains explicit app/package work.
- CRUD bulk actions are client-side by default: generated list pages create a page-local `listBulkActions.js`, wire `useCrudListBulkActions(...)`, and keep selection controls hidden until actions are declared.
- `page.supporting-content` is now a semantic supporting region in the default shell. Compact layout renders it as a closed bottom sheet; medium/expanded layouts render it as a closed side panel.
