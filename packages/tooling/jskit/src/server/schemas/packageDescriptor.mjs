...
function normalizeRuntimeServerConfig(runtimeSource, packageId) {
  ...
  const providerEntrypointRaw = String(serverRuntime.providerEntrypoint || "").trim();
  const providerExportRaw = String(serverRuntime.providerExport || "").trim();

  if (!providerEntrypointRaw) {
    throw createCliError(
      `Package ${packageId} runtime.server.providerEntrypoint is required when runtime.server is declared.`
    );
  }
  if (providerExportRaw && !/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(providerExportRaw)) {
    throw createCliError(`Package ${packageId} runtime.server.providerExport is invalid: ${providerExportRaw}`);
  }

  return {
    providerEntrypoint: normalizeRelativePath(providerEntrypointRaw),
    providerExport: providerExportRaw
  };
}
