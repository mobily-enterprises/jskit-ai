...
function createBillingCatalogCore({
createError: createErrorOverride,
  parsePositiveInteger: parsePositiveIntegerOverride,
  isDuplicateEntryError: isDuplicateEntryErrorOverride,
  assertEntitlementValueOrThrow: assertEntitlementValueOrThrowOverride
} = {}) {
  const createError =
    typeof createErrorOverride === "function"
      ? createErrorOverride
  ...
  const parsePositiveInteger =
    typeof parsePositiveIntegerOverride === "function"
  ...
  const isDuplicateEntryError =
    typeof isDuplicateEntryErrorOverride === "function"
      ? isDuplicateEntryErrorOverride
      : (error) => String(error?.code || "") === "ER_DUP_ENTRY";
  const assertEntitlementValueOrThrow =
    typeof assertEntitlementValueOrThrowOverride === "function" ? assertEntitlementValueOrThrowOverride : () => {};
...
function mapBillingPlanDuplicateError(error) {
  ...
  if (normalizedMessage.includes("uq_billing_plans_code")) {
    return createError(409, "Billing plan code already exists.");
  }
  if (normalizedMessage.includes("uq_billing_plans_checkout_provider_price")) {
    return createError(409, "Provider price id is already mapped to another plan.");
  }

  return createError(409, "Billing catalog conflict.");
}
...
function mapBillingProductDuplicateError(error) {
  ...
  if (normalizedMessage.includes("uq_billing_products_code")) {
    return createError(409, "Billing product code already exists.");
  }
  if (normalizedMessage.includes("uq_billing_products_provider_price")) {
    return createError(409, "Provider price id is already mapped to another product.");
  }
...
function ensureBillingCatalogRepository(billingRepository) {
  if (!billingRepository) {
    throw createError(501, "Console billing catalog is not available.");
  }
...
function ensureBillingProductCatalogRepository(billingRepository) {
  if (!billingRepository) {
    throw createError(501, "Console billing product catalog is not available.");
  }
...
export { createBillingCatalogCore };
