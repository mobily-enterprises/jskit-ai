...
function createMembershipsRepository(dbClient) {
  ...
  async function repoFindByUserId(userId, options = {}) {
    ...
    const row = await client("console_memberships").where({ user_id: userId }).first();
  ...
  async function repoCountActiveMembers(options = {}) {
    const client = resolveClient(options);
    const row = await client("console_memberships").where({ status: "active" }).count({ total: "id" }).first();
  ...
  async function repoInsert(membership, options = {}) {
    const client = resolveClient(options);
    const now = new Date();
    const [id] = await client("console_memberships").insert({
    ...
    const row = await client("console_memberships").where({ id }).first();
    return mapMembershipRowRequired(row);
  }
  ...
  async function repoEnsureActiveByUserId(userId, roleId, options = {}) {
    ...
    await client("console_memberships")
...
function createRepository(dbClient) {
  ...
  return createMembershipsRepository(dbClient);
}
