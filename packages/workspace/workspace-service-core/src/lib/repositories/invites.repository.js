...
function createWorkspaceInvitesRepository(dbClient) {
  async function repoInsert(invite, options = {}) {
    ...
    const [id] = await client("workspace_invites").insert({
      ...
      expires_at: toDatabaseDateTimeUtc(new Date(invite.expiresAt)),
  ...
  async function repoListPendingByWorkspaceId(workspaceId, options = {}) {
    ...
    const rows = await createInviteBaseQuery(client)
      ...
      .andWhere("wi.expires_at", ">", toDatabaseDateTimeUtc(new Date()))
  ...
  async function repoDeleteArtifactsOlderThan(cutoffDate, batchSize = 1000, options = {}) {
    return deleteRowsOlderThan({
      client: resolveRepoClient(dbClient, options),
...
function createRepository(dbClient) {
  ...
  return createWorkspaceInvitesRepository(dbClient);
}
