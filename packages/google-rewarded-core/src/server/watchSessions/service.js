...
function createService({ googleRewardedWatchSessionsRepository } = {}) {
  ...
  async function queryDocuments(query = {}, options = {}) {
    return returnJsonApiDocument(await googleRewardedWatchSessionsRepository.queryDocuments(query, {
  ...
  async function createDocument(payload = {}, options = {}) {
    return returnJsonApiDocument(await googleRewardedWatchSessionsRepository.createDocument(payload, {
      trx: options?.trx || null,
      context: options?.context || null
    }));
  }
  ...
  async function patchDocumentById(recordId, payload = {}, options = {}) {
    return returnJsonApiDocument(return404IfNotFound(await googleRewardedWatchSessionsRepository.patchDocumentById(recordId, payload, {
      trx: options?.trx || null,
      context: options?.context || null
    })));
  }
  ...
  async function deleteDocumentById(recordId, options = {}) {
    return googleRewardedWatchSessionsRepository.deleteDocumentById(recordId, {
      trx: options?.trx || null,
      context: options?.context || null
    });
  }
