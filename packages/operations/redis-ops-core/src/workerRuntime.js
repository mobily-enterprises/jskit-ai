...
import { RETENTION_QUEUE_NAME, createWorkerRedisPrefix } from "./workerConstants.js";
import { createRetentionDeadLetterQueue, enqueueRetentionDeadLetterJob } from "./deadLetterQueue.js";
import { createWorkerRedisConnection, closeWorkerRedisConnection } from "./workerRedisConnection.js";
...
function createWorkerRuntime({
  ...
  if (typeof createRetentionSweepProcessorImpl !== "function") {
    throw new Error("createRetentionSweepProcessorImpl is required.");
  }
  if (typeof isLockHeldError !== "function") {
    throw new Error("isLockHeldError is required.");
  }
  ...
  async function enqueueDeadLetterJob(job, error, { includeLockHeldError = false } = {}) {
    if (isLockHeldError(error) && !includeLockHeldError) {
      return;
    }
