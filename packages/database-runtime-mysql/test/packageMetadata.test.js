...
test("database-runtime-mysql contributes canonical synthetic MariaDB CI requirements", () => {
  ...
  assert.equal(packageMetadata.ci.environment.DB_PORT, "33060");
  assert.match(packageMetadata.ci.environment.DB_PASSWORD, /ci_only/u);

  const service = packageMetadata.ci.services.find((entry) => entry.id === "mariadb");
  assert.equal(service.image, "mariadb:11.4");
  assert.equal(service.environment.MARIADB_DATABASE, packageMetadata.ci.environment.DB_NAME);
  assert.equal(service.environment.MARIADB_USER, packageMetadata.ci.environment.DB_USER);
  assert.equal(service.environment.MARIADB_PASSWORD, packageMetadata.ci.environment.DB_PASSWORD);
