...
function createAttachmentsRepository(dbClient) {
  ...
  async function repoInsertReserved(payload, options = {}) {
    ...
    const [id] = await client("chat_attachments").insert({
      ...
      upload_expires_at: payload?.uploadExpiresAt ? toDatabaseDateTimeUtc(new Date(payload.uploadExpiresAt)) : null,
  ...
  async function repoApplyStatusTransition(attachmentId, nextStatus, allowedCurrentStatuses, patch = {}, options = {}) {
    ...
    if (Object.hasOwn(patch, "uploadExpiresAt")) {
      dbPatch.upload_expires_at = patch.uploadExpiresAt ? toDatabaseDateTimeUtc(new Date(patch.uploadExpiresAt)) : null;
    }
    if (Object.hasOwn(patch, "processedAt")) {
      dbPatch.processed_at = patch.processedAt ? toDatabaseDateTimeUtc(new Date(patch.processedAt)) : null;
    }
    ...
    if (Object.hasOwn(patch, "deletedAt")) {
      dbPatch.deleted_at = patch.deletedAt ? toDatabaseDateTimeUtc(new Date(patch.deletedAt)) : null;
    }
