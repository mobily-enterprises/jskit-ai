...
function createParticipantsRepository(dbClient) {
    ...
    const [id] = await client("chat_thread_participants").insert({
      ...
      status:
        String(payload?.status || "")
          .trim()
          .toLowerCase() || "active",
      joined_at: toDatabaseDateTimeUtc(payload?.joinedAt ? new Date(payload.joinedAt) : now),
      left_at: payload?.leftAt ? toDatabaseDateTimeUtc(new Date(payload.leftAt)) : null,
      removed_by_user_id: parsePositiveInteger(payload?.removedByUserId),
      mute_until: payload?.muteUntil ? toDatabaseDateTimeUtc(new Date(payload.muteUntil)) : null,
      archived_at: payload?.archivedAt ? toDatabaseDateTimeUtc(new Date(payload.archivedAt)) : null,
      pinned_at: payload?.pinnedAt ? toDatabaseDateTimeUtc(new Date(payload.pinnedAt)) : null,
      ...
      last_read_at: payload?.lastReadAt ? toDatabaseDateTimeUtc(new Date(payload.lastReadAt)) : null,
      draft_text: payload?.draftText == null ? null : String(payload.draftText),
      draft_updated_at: payload?.draftUpdatedAt ? toDatabaseDateTimeUtc(new Date(payload.draftUpdatedAt)) : null,
  ...
  async function repoUpdateByThreadIdAndUserId(threadId, userId, patch = {}, options = {}) {
    ...
    if (Object.hasOwn(patch, "leftAt")) {
      dbPatch.left_at = patch.leftAt ? toDatabaseDateTimeUtc(new Date(patch.leftAt)) : null;
    }
    ...
    if (Object.hasOwn(patch, "muteUntil")) {
      dbPatch.mute_until = patch.muteUntil ? toDatabaseDateTimeUtc(new Date(patch.muteUntil)) : null;
    }
    ...
    if (Object.hasOwn(patch, "pinnedAt")) {
      dbPatch.pinned_at = patch.pinnedAt ? toDatabaseDateTimeUtc(new Date(patch.pinnedAt)) : null;
    }
    ...
    if (Object.hasOwn(patch, "lastReadAt")) {
      dbPatch.last_read_at = patch.lastReadAt ? toDatabaseDateTimeUtc(new Date(patch.lastReadAt)) : null;
    }
    ...
    if (Object.hasOwn(patch, "draftUpdatedAt")) {
      dbPatch.draft_updated_at = patch.draftUpdatedAt ? toDatabaseDateTimeUtc(new Date(patch.draftUpdatedAt)) : null;
    }
  ...
  async function repoUpdateReadCursorMonotonic(threadId, userId, patch = {}, options = {}) {
    ...
    const nextReadAt = patch?.lastReadAt ? toDatabaseDateTimeUtc(new Date(patch.lastReadAt)) : null;
  ...
  async function repoRepairPointersForThread(threadId, { lastMessageSeq = null } = {}, options = {}) {
    ...
    const nowValue = toDatabaseDateTimeUtc(new Date());
...
export { createParticipantsRepository as createRepository, createParticipantsRepository, __testables };
