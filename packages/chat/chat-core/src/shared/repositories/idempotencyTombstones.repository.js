...
import { normalizeBatchSize } from "@jskit-ai/jskit-knex/retention";
...
function createIdempotencyTombstonesRepository(dbClient) {
  ...
  async function repoInsertForDeletedMessage(payload, options = {}) {
    ...
    const insertPayload = {
      ...
      deleted_at: toDatabaseDateTimeUtc(normalized.payload.deletedAt),
      expires_at: toDatabaseDateTimeUtc(normalized.payload.expiresAtDate),
    ...
    if (existingRow) {
      ...
      if (laterExpiresAt) {
        updatePatch.expires_at = toDatabaseDateTimeUtc(laterExpiresAt);
      }
      if (
      ...
      if (Object.keys(updatePatch).length > 0) {
        updatePatch.updated_at = toDatabaseDateTimeUtc(new Date());
  ...
  async function repoListExpired(batchSize = 1000, now = new Date(), options = {}) {
    ...
    const threshold = toDatabaseDateTimeUtc(now);
  ...
  async function repoCountActiveByExpiryBucket(startAt, endAt, options = {}) {
    const client = resolveClient(dbClient, options);
    const start = startAt ? toDatabaseDateTimeUtc(new Date(startAt)) : null;
    const end = endAt ? toDatabaseDateTimeUtc(new Date(endAt)) : null;
