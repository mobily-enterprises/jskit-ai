...
import { __testables } from "./actionDefinitions.js";
...
test("normalizeAssistantToolConfig accepts section-map inputValidator", () => {
  const assistantTool = __testables.normalizeAssistantToolConfig({
  ...
  assert.equal(assistantTool?.inputJsonSchema?.type, "object");
  assert.ok(Object.hasOwn(assistantTool?.inputJsonSchema?.properties || {}, "patch"));
});
