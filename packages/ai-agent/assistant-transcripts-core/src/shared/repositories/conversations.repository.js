...
function applyListFilters(query, filters = {}) {
  ...
  if (filters.from) {
    query.where("c.started_at", ">=", toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(filters.from)));
  }
  if (filters.to) {
    query.where("c.started_at", "<=", toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(filters.to)));
  }
...
function createConversationsRepository(dbClient) {
  ...
  async function repoInsert(payload, options = {}) {
    ...
    const [id] = await client("ai_conversations").insert({
      ...
      started_at: toDatabaseDateTimeUtc(startedAt),
      ended_at: payload?.endedAt ? toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(payload.endedAt)) : null,
  ...
  async function repoUpdateById(conversationId, patch = {}, options = {}) {
    ...
    if (Object.hasOwn(patch, "endedAt")) {
      dbPatch.ended_at = patch.endedAt ? toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(patch.endedAt)) : null;
    }
...
export { createConversationsRepository as createRepository, createConversationsRepository, __testables };
