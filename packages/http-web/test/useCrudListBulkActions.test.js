...
test("useCrudListBulkActions manages selection and executes action context", async () => {
  const { defineCrudListBulkActions } = await import("@jskit-ai/http-web/client/bulkActions");
  const { useCrudListBulkActions } = await import("@jskit-ai/http-web/client/composables/useCrudListBulkActions");
  const calls = [];
