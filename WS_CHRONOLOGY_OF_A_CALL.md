...
12. **Socket.IO realtime fans out by room `w:<workspaceId>:t:<topic>`** (`server/realtime/registerSocketIoRealtime.js:634`)
...
function fanoutEvent(eventEnvelope) {
  ...
  if (!Number.isInteger(workspaceId) || workspaceId < 1 || !topic) return;

  io.to(buildRoomName(workspaceId, topic)).emit(SOCKET_IO_MESSAGE_EVENT, {
