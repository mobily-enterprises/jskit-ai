...
} from "../tooling/jskit-catalog/scripts/pattern-assets.mjs";
...
const PATTERN_INDEX_PATH = path.join(REFERENCE_AUTOGEN_ROOT, "PATTERN_INDEX.md");
...
const PUBLIC_PATTERN_ROOT = path.join(GUIDE_SITE_ROOT, "patterns");
...
const JSKIT_SKILL_GUIDES = Object.freeze({
  ...
  "framework/ui-operations.md": "ui-operations.md"
});
const REPOSITORY_WEB_ROOT = "https://github.com/mobily-enterprises/jskit-ai";
...
const SKIP_RELATIVE_DIRECTORIES = new Set([
  ".vitepress/cache",
  ".vitepress/dist",
  "skills/jskit/references/patterns"
]);
...
function toGeneratedSkillReference(sourceText, { commandName, sourcePath }) {
  const normalized = stripFrontMatter(String(sourceText || "")).trim();
  return `<!-- Generated from \`${sourcePath}\` by \`${commandName}\`. -->\n\n${normalized}\n`;
}

function rewriteSkillGuideLinks(sourceText = "") {
  return String(sourceText || "")
    .replace(
      /\[[^\]]*source pattern(?:\s+library)?\]\(\/patterns\/\)/giu,
      "[bundled source pattern index](pattern-index.md)"
    )
    .replace(/\(\.\/material-3\)/gu, "(material-3.md)");
}
...
async function clearAgentDocsOutputs() {
  ...
  await rm(PUBLIC_PATTERN_ROOT, { recursive: true, force: true });
  await rm(JSKIT_SKILL_PATTERN_INDEX_PATH, { force: true });
  await rm(JSKIT_SKILL_PATTERNS_ROOT, { recursive: true, force: true });
  for (const targetFile of Object.values(JSKIT_SKILL_GUIDES)) {
    await rm(path.join(JSKIT_SKILL_REFERENCES_ROOT, targetFile), { force: true });
  }
}
...
function renderPatternIndex(patterns, commandName, { patternLinkPrefix }) {
  const lines = [
  ...
  if (patterns.length < 1) {
    lines.push("No source patterns are currently published.", "");
    return lines.join("\n");
  }
  ...
  for (const pattern of patterns) {
    ...
    lines.push(`- Keywords: ${pattern.keywords.map((keyword) => `\`${keyword}\``).join(", ")}`);
    lines.push(`- Owner: \`${pattern.packageId}@${pattern.packageVersion}\``);
    const patternPath = `${patternLinkPrefix}/${pattern.id}`;
    lines.push(`- Read: [PATTERN.md](${patternPath}/PATTERN.md)`);
    lines.push(`- Examples: [example/](${patternPath}/example/)`);
    if (pattern.requires.length > 0) {
      lines.push(`- Requires: ${pattern.requires.map((packageId) => `\`${packageId}\``).join(", ")}`);
    }
    lines.push("");
  }
...
function stripFrontMatter(sourceText = "") {
  ...
  return closingIndex < 0
    ? normalized.trim()
    : normalized.slice(closingIndex + 5).trim();
}

function patternRepositoryPath(pattern, suffix) {
  return normalizeMarkdownPath(path.relative(
    REPO_ROOT,
    path.join(pattern.ownerRoot, suffix)
  ));
}

function renderPublicPatternIndex(patterns, commandName) {
  const lines = [
    "---",
    ...
    "---",
    "",
    `<!-- Generated by \`${commandName}\` from package-owned patterns. Do not edit manually. -->`,
    "",
    "# Source patterns",
    "",
  ...
  for (const pattern of patterns) {
    lines.push(`## [${pattern.title}](/patterns/${pattern.id})`);
    lines.push("");
    lines.push(pattern.summary);
    lines.push("");
    lines.push(`- Id: \`${pattern.id}\``);
...
function renderPublicPattern(pattern, sourceText, commandName) {
  ...
  const lines = [
    ...
    `description: ${JSON.stringify(pattern.summary)}`,
    "---",
    "",
    `<!-- Generated by \`${commandName}\` from \`${documentRepositoryPath}\`. Do not edit manually. -->`,
    "",
    body,
    "",
    "## Packaged source",
    "",
    `- Owner: \`${pattern.packageId}@${pattern.packageVersion}\``,
    `- [Browse PATTERN.md](${REPOSITORY_WEB_ROOT}/blob/main/${documentRepositoryPath})`,
    `- [Browse the complete example tree](${REPOSITORY_WEB_ROOT}/tree/main/${exampleRepositoryPath})`,
    ""
  ];
  return lines.join("\n");
}

async function buildPatternIndex(commandName) {
  const patterns = [];
  for (const workspace of await collectWorkspaceDirectories()) {
    let packageJson;
    try {
      packageJson = JSON.parse(await readFile(path.join(workspace.workspaceDir, "package.json"), "utf8"));
    } catch (error) {
      if (error?.code === "ENOENT") {
        continue;
      }
    ...
    if (!String(packageJson?.name || "").trim() || !String(packageJson?.version || "").trim()) {
      continue;
    }

    const workspacePatterns = await discoverPackagePatterns({
      packageRoot: workspace.workspaceDir,
      packageJson
    });
    patterns.push(...workspacePatterns.map((pattern) => ({
      ...pattern,
      ownerRoot: workspace.workspaceDir
    })));
  }
  ...
  await writeTextFile(
    PATTERN_INDEX_PATH,
    renderPatternIndex(patterns, commandName, {
      patternLinkPrefix: "../../skills/jskit/references/patterns"
    })
  );
  await writeTextFile(
    JSKIT_SKILL_PATTERN_INDEX_PATH,
    renderPatternIndex(patterns, commandName, { patternLinkPrefix: "patterns" })
  );
  await writeTextFile(
    path.join(PUBLIC_PATTERN_ROOT, "index.md"),
    renderPublicPatternIndex(patterns, commandName)
  );

  for (const pattern of patterns) {
    const bundledPatternRoot = path.join(JSKIT_SKILL_PATTERNS_ROOT, pattern.id);
    const sourceDocumentPath = path.join(pattern.ownerRoot, pattern.documentPath);
    const sourceExamplePath = path.join(pattern.ownerRoot, pattern.examplePath);
    const sourceText = await readFile(sourceDocumentPath, "utf8");
    await mkdir(bundledPatternRoot, { recursive: true });
    await cp(sourceDocumentPath, path.join(bundledPatternRoot, "PATTERN.md"));
...
async function buildGuideOutputs(commandName) {
  ...
  for (const sourceFilePath of markdownFilePaths) {
    ...
    const compressedText = compressGuideMarkdown(sourceText);

    const outputText = toGeneratedGuideText(compressedText, {
      commandName,
      sourcePath: sourceMarkdownPath
    });
    await writeTextFile(path.join(GUIDE_AGENT_ROOT, relativePath), outputText);
    const skillTarget = JSKIT_SKILL_GUIDES[relativePath];
    if (skillTarget) {
      await writeTextFile(
        path.join(JSKIT_SKILL_REFERENCES_ROOT, skillTarget),
        toGeneratedSkillReference(rewriteSkillGuideLinks(compressedText), {
          commandName,
          sourcePath: sourceMarkdownPath
        })
      );
...
async function main() {
  ...
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, PUBLIC_PATTERN_ROOT))}\n`);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, JSKIT_SKILL_PATTERN_INDEX_PATH))}\n`);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, JSKIT_SKILL_PATTERNS_ROOT))}\n`);
  for (const targetFile of Object.values(JSKIT_SKILL_GUIDES)) {
    process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, path.join(JSKIT_SKILL_REFERENCES_ROOT, targetFile)))}\n`);
  }
}
