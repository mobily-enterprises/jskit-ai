...
const AI_DOCS_ROOT = path.join(REPO_ROOT, "ai-docs");
const AI_DOCS_AUTOGEN_ROOT = path.join(AI_DOCS_ROOT, "autogen");
const KERNEL_SHARED_ROOT = path.join(REPO_ROOT, "packages", "kernel", "shared");
const STARTUP_KERNEL_MAP_PATH = path.join(AI_DOCS_AUTOGEN_ROOT, "KERNEL_MAP.md");
const AI_DOCS_README_PATH = path.join(AI_DOCS_AUTOGEN_ROOT, "README.md");
...
async function clearAiDocsOutputs() {
  await rm(path.join(AI_DOCS_AUTOGEN_ROOT, "packages"), { recursive: true, force: true });
  await rm(path.join(AI_DOCS_AUTOGEN_ROOT, "tooling"), { recursive: true, force: true });
  await rm(STARTUP_KERNEL_MAP_PATH, { force: true });
  await rm(AI_DOCS_README_PATH, { force: true });

  // Clean the previous root-level generated layout during the ai-docs/autogen migration.
  await rm(path.join(AI_DOCS_ROOT, "packages"), { recursive: true, force: true });
  await rm(path.join(AI_DOCS_ROOT, "tooling"), { recursive: true, force: true });
  await rm(path.join(AI_DOCS_ROOT, "KERNEL_MAP.md"), { force: true });
  await rm(path.join(AI_DOCS_ROOT, "README.md"), { force: true });
}
...
async function buildStartupKernelMap(commandName) {
  ...
  const outputText = renderMap({
    ...
    introLines: [
      ...
      "For the full repo inventory, read `ai-docs/autogen/README.md` and the package maps it lists.",
      "",
      "## Reuse Order",
...
function renderAiDocsReadme(workspaceMaps, commandName) {
...
async function buildAiDocs(commandName) {
  ...
  await writeTextFile(
    AI_DOCS_README_PATH,
    renderAiDocsReadme(workspaceMaps, commandName)
  );
}

async function main() {
  const commandName = "npm run ai-docs:build";
  await clearAiDocsOutputs();
  await mkdir(AI_DOCS_ROOT, { recursive: true });
  await mkdir(AI_DOCS_AUTOGEN_ROOT, { recursive: true });
  await buildStartupKernelMap(commandName);
  await buildAiDocs(commandName);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, STARTUP_KERNEL_MAP_PATH))}\n`);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, AI_DOCS_README_PATH))}\n`);
}
