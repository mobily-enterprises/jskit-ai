...
const AGENT_INDEX_ROOT = path.join(REPO_ROOT, "agent-index");
const KERNEL_SHARED_ROOT = path.join(REPO_ROOT, "packages", "kernel", "shared");
const STARTUP_KERNEL_MAP_PATH = path.join(AGENTS_ROOT, "KERNEL_MAP.md");
const AGENT_INDEX_README_PATH = path.join(AGENT_INDEX_ROOT, "README.md");
...
function renderAgentIndexReadme(workspaceMaps, commandName) {
...
async function buildAgentIndex(commandName) {
  const workspaceDirectories = await collectWorkspaceDirectories();
  const workspaceMaps = [];

  await rm(AGENT_INDEX_ROOT, { recursive: true, force: true });
  await mkdir(AGENT_INDEX_ROOT, { recursive: true });
  ...
  await writeTextFile(
    AGENT_INDEX_README_PATH,
    renderAgentIndexReadme(workspaceMaps, commandName)
  );
}

async function main() {
  const commandName = "npm run agents:build-index";
  await buildStartupKernelMap(commandName);
  await buildAgentIndex(commandName);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, STARTUP_KERNEL_MAP_PATH))}\n`);
  process.stdout.write(`Wrote ${normalizeMarkdownPath(path.relative(REPO_ROOT, AGENT_INDEX_README_PATH))}\n`);
}
