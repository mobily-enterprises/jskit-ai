...
test("commandLintPackages passes when metadata tokens match provider bindings", async () => {
  ...
  const status = await handlers.commandLintPackages({
    options: {
      json: false,
      checkDiLabels: true
    },
  ...
  assert.match(output, /Package metadata lint passed\./);
  assert.match(output, /DI label parity check passed\./);
});

test("commandLintPackages fails on unresolved token expressions in strict mode", async () => {
...
test("commandLintPackages --json includes strict DI issue payload", async () => {
  ...
  assert.ok(issues.some((issue) => issue.code === "metadata-token-unused"));
});
