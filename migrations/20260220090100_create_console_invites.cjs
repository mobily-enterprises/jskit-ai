exports.up = async function up(knex) {
  await knex.schema.createTable("console_invites", (table) => {
    ...
    table.index(["email", "status"], "idx_console_invites_email_status");
    table.index(["expires_at", "status"], "idx_console_invites_expires_status");
  });

  await knex.raw(`
    ALTER TABLE console_invites
  ...
  await knex.raw(`
    ALTER TABLE console_invites
    ADD UNIQUE INDEX uq_console_invites_pending_email (pending_email)
  `);
};

exports.down = async function down(knex) {
  await knex.schema.dropTableIfExists("console_invites");
};
