...
| 2.1 usage counters + windowed limits | `billing_usage_counters` table + repository increment/find/list operations + `billingService.recordUsage` | partial | Counter persistence and UTC window resolution are implemented; strict cross-feature enforcement rollout remains pending. |
| 2.1 clean app-facing limitations contract | `billingService.getLimitations` + `GET /api/billing/limitations` + client API surface | partial | Contract is available for app consumption; feature-by-feature adoption/enforcement wiring remains pending. |
...
| 2.2 workspace billing UX (user-friendly) | `GET /api/billing/timeline` + workspace billing view/route + admin-shell nav wiring | partial | User-friendly timeline/status surface shipped for workspace billing activity; full billing management UX and advanced reporting are still pending. |
| 2.3 metered/hybrid component enablement | Subscription checkout pricing now resolves base sellable price plus active metered components for usage/hybrid plans | partial | Checkout remains deterministic and currency-scoped; licensed optional-component quantity selection is still pending. |
| 2.3 one-off billing flows | `POST /api/billing/checkout` (`checkoutType=one_off`) + `POST /api/billing/payment-links` now support catalog/ad-hoc one-off purchases with invoice projection into `billing_invoices` | partial | Core orchestration is implemented; richer catalog UX and licensed optional-component quantity selection remain pending. |
