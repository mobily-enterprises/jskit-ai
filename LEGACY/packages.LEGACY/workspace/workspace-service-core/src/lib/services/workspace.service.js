...
function canAccessAdminWorkspace(context = {}) {
  const membership = context.membership && typeof context.membership === "object" ? context.membership : null;
...
function createDefaultSurfaceResolver() {
  function normalizeSurfaceId(surfaceId) {
    return normalizeSupportedSurfaceId(surfaceId);
  }

  function resolveSurfaceById(surfaceId) {
    const normalizedSurfaceId = normalizeSurfaceId(surfaceId);
    const canAccessWorkspace =
      normalizedSurfaceId === "app"
        ? canAccessAppWorkspace
        : normalizedSurfaceId === "admin"
...
function createService({
  ...
  const effectiveSurfaceResolver =
    surfaceResolver &&
    typeof surfaceResolver.normalizeSurfaceId === "function" &&
    typeof surfaceResolver.resolveSurfaceById === "function"
      ? surfaceResolver
      : createDefaultSurfaceResolver();
  const resolveRequestSurfaceIdOptions = {
  ...
  async function evaluateWorkspaceAccess({ user, surfaceId, workspace, membership, workspaceSettingsCache }) {
    ...
    const normalizedSurfaceId = effectiveSurfaceResolver.normalizeSurfaceId(surfaceId);
    const surface = effectiveSurfaceResolver.resolveSurfaceById(normalizedSurfaceId);
