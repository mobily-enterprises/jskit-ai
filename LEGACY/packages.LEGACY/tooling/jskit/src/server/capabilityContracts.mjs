...
function extractModuleNamedExports(source) {
  ...
  for (const match of input.matchAll(/export\s+\*\s+from\s+["']([^"']+)["']/g)) {
    const specifier = String(match[1] || "").trim();
    if (specifier) {
      starReexports.add(specifier);
    }
  }
  ...
  return {
    symbols: toSortedUniqueStrings([...symbols]),
    starReexports: [...starReexports]
  };
}

async function resolveLocalReexportPath(fromPath, specifier) {
...
async function loadEntrypointNamedExports(resolvedPath, moduleCache, resolutionStack = new Set()) {
  ...
  const parsed = extractModuleNamedExports(source);
  const symbolSet = new Set(parsed.symbols);
  const nextStack = new Set(resolutionStack);
  nextStack.add(resolvedPath);

  for (const specifier of parsed.starReexports) {
    const reexportPath = await resolveLocalReexportPath(resolvedPath, specifier);
    if (!reexportPath) {
      continue;
    }
    const childSymbols = await loadEntrypointNamedExports(reexportPath, moduleCache, nextStack);
    for (const symbol of childSymbols) {
      symbolSet.add(symbol);
    }
  }

  const symbols = toSortedUniqueStrings([...symbolSet]);
