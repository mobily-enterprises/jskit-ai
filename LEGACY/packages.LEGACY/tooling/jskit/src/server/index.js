...
async function lintDescriptors({ appRoot }) {
  ...
  const serverProviderIssues = await validateServerProviderContracts(packages);
  if (serverProviderIssues.length > 0) {
    throw createConflictError(
      "server-provider-contract",
      "Descriptor lint failed server provider contract checks.",
      serverProviderIssues.map((message) => createIssue(message))
    );
  }
  ...
  for (const packageEntry of orderedEntries) {
    ...
    if (!runtimeServer) {
      if (providesServerCapabilities) {
        issues.push(
          `Package ${packageId} provides server route capabilities but does not declare runtime.server.providerEntrypoint.`
        );
      }
      continue;
    }

    const providerEntrypoint = String(runtimeServer.providerEntrypoint || "").trim();
    const providerExport = String(runtimeServer.providerExport || "").trim();
    if (!providerEntrypoint) {
      issues.push(`Package ${packageId} runtime.server.providerEntrypoint is required when runtime.server is declared.`);
      continue;
    }
    if (providerExport && !/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(providerExport)) {
      issues.push(`Package ${packageId} runtime.server.providerExport is invalid: ${providerExport}.`);
      continue;
    }

    const absoluteEntrypointPath = path.resolve(packageEntry.packageRoot, providerEntrypoint);
    ...
    if (!isInsidePackageRoot) {
      issues.push(`Package ${packageId} runtime.server.providerEntrypoint escapes package root (${providerEntrypoint}).`);
      continue;
    }
    if (!(await fileExists(absoluteEntrypointPath))) {
      issues.push(`Package ${packageId} runtime.server.providerEntrypoint not found: ${providerEntrypoint}.`);
      continue;
    }

    let providerModule = null;
    try {
      providerModule = await import(pathToFileURL(absoluteEntrypointPath).href + `?t=${Date.now()}_${Math.random()}`);
    } catch (error) {
      issues.push(
        `Package ${packageId} failed loading runtime.server.providerEntrypoint ${providerEntrypoint}: ${error?.message || String(error)}`
      );
      continue;
    }

    if (providerExport) {
      if (!Object.prototype.hasOwnProperty.call(providerModule, providerExport)) {
        issues.push(`Package ${packageId} runtime.server.providerExport ${providerExport} was not found.`);
        continue;
      }
      if (!isProviderDefinition(providerModule[providerExport])) {
        issues.push(`Package ${packageId} runtime.server.providerExport ${providerExport} must be a provider class/function.`);
        continue;
      }
      continue;
    }

    const discoveredProviders = Object.values(providerModule).filter((entry) => isProviderDefinition(entry));
    if (discoveredProviders.length < 1) {
      issues.push(`Package ${packageId} runtime.server provider entrypoint did not export any provider classes/functions.`);
    }
  }
