...
function applyExportFilters(query, filters = {}) {
  ...
  if (filters.from) {
    query.where("m.created_at", ">=", toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(filters.from)));
  }
  if (filters.to) {
    query.where("m.created_at", "<=", toDatabaseDateTimeUtc(normalizeCutoffDateOrThrow(filters.to)));
  }
...
function createMessagesRepository(dbClient) {
  ...
  async function repoInsert(payload, options = {}) {
    ...
    const [id] = await client("ai_messages").insert({
      ...
      created_at: toDatabaseDateTimeUtc(createdAt)
    });
