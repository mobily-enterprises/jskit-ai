...
function createMessagesRepository(dbClient) {
  async function repoInsert(payload, options = {}) {
    ...
    const [id] = await client("chat_messages").insert({
      ...
      sent_at: toDatabaseDateTimeUtc(payload?.sentAt ? new Date(payload.sentAt) : now),
  ...
  async function repoUpdateById(messageId, patch = {}, options = {}) {
    ...
    if (Object.hasOwn(patch, "editedAt")) {
      dbPatch.edited_at = patch.editedAt ? toDatabaseDateTimeUtc(new Date(patch.editedAt)) : null;
    }
...
export { createMessagesRepository as createRepository, createMessagesRepository, __testables };
