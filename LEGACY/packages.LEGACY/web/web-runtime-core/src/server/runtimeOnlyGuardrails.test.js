...
const ALLOWED_REFERENCE_PATHS = new Set(["tests/architecture/client-architecture.guardrails.test.mjs"]);
...
test("web-runtime-core apiClients import path is not used outside migration docs", () => {
  ...
  for (const relativePath of collectScanTargets(repoRoot)) {
    ...
    if (ALLOWED_REFERENCE_PATHS.has(relativePath.replaceAll(path.sep, "/"))) {
      continue;
    }
