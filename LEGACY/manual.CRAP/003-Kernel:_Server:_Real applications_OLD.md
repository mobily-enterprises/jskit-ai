...
- Stage 6 action input was raw transport payload (`request.body`).
- Stage 6 actions called `qualificationService.qualify(rawPayload)`.
- `qualify(...)` bundled multiple steps together:
  - normalize raw payload
  - validate domain rules
  - compute score/segment/follow-up
  - return one combined `qualified` object
- Stage 7 moves normalization earlier (route contract `normalize`), so actions now receive already-normalized payload.
- Stage 7 actions do not call `qualify(...)`; they run explicit domain steps (`validate`, `score`, `segment`, `followupPlan`) on normalized input.
- This avoids doing normalization twice and keeps transport shaping in the transport layer.

Before vs after mental model:

1. Stage 6 flow:
   - `request.body` -> action -> `qualify(raw)` -> `qualified.*`
2. Stage 7 flow:
   - `request.body` -> contract `normalize` -> `request.input.body` -> action domain steps

What did not change:

- use-case intent (intake persists, preview does not)
- result envelope style (`{ ok, status, data }` / failure with `{ ok: false, ... }`)
- repository interaction pattern (duplicate check + save for intake)
...
// Stage 6 style
const qualified = this.qualificationService.qualify(payload);
if (!qualified.ok) return { ok: false, ... };
const duplicate = this.contactRepository.findByEmail(qualified.normalized.email);
const created = this.contactRepository.save({ ...qualified.normalized, score: qualified.score });
...
const created = this.contactRepository.save({ ...normalizedPayload, score, segment });
```
