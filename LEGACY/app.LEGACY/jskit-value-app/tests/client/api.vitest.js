...
describe("client api transport", () => {
  ...
  it("calls workspace and security wrapper endpoints and encodes identifiers", async () => {
    ...
    expect(urls).toContain("/api/admin/workspace/settings");
    expect(urls).toContain("/api/admin/workspace/roles");
    expect(urls).toContain("/api/admin/workspace/members");
    expect(urls).toContain("/api/admin/workspace/members/user%2Fid/role");
    expect(urls).toContain("/api/admin/workspace/invites");
    expect(urls).toContain("/api/admin/workspace/invites/invite%20id%2F2");
    ...
    expect(urls).toContain("/api/console/billing/entitlement-definitions?includeInactive=true&code=workspace.project.monthly");
    expect(urls).toContain("/api/console/billing/entitlement-definitions/3");
    expect(urls).toContain("/api/console/billing/plans/12/archive");
    expect(urls).toContain("/api/console/billing/plans/12/unarchive");
    expect(urls).toContain("/api/console/billing/products/21/archive");
    expect(urls).toContain("/api/console/billing/products/21/unarchive");
    expect(urls).toContain(
      "/api/console/billing/purchases?page=1&pageSize=20&status=confirmed&provider=stripe&purchaseKind=subscription_invoice"
    );
    expect(urls).toContain("/api/console/billing/purchases/44/refund");
    expect(urls).toContain("/api/console/billing/purchases/44/void");
    expect(urls).toContain("/api/console/billing/purchases/44/corrections");
    expect(urls).toContain("/api/console/billing/plan-assignments?page=1&pageSize=20&status=current");
    expect(urls).toContain("/api/console/billing/plan-assignments");
    expect(urls).toContain("/api/console/billing/plan-assignments/55");
    expect(urls).toContain("/api/console/billing/plan-assignments/55/cancel");
    expect(urls).toContain("/api/console/billing/subscriptions?page=1&pageSize=20&status=active");
    expect(urls).toContain("/api/console/billing/subscriptions/sub_123/change-plan");
    expect(urls).toContain("/api/console/billing/subscriptions/sub_123/cancel");
    expect(urls).toContain("/api/console/billing/subscriptions/sub_123/cancel-at-period-end");
    ...
    const consoleRefundCall = global.fetch.mock.calls.find(
      ([url, options]) =>
        url === "/api/console/billing/purchases/44/refund" && String(options?.method || "").toUpperCase() === "POST"
    );
    ...
    const workspaceDefaultPaymentMethodCall = global.fetch.mock.calls.find(
      ([url, options]) =>
        url === "/api/billing/payment-methods/31/default" && String(options?.method || "").toUpperCase() === "POST"
    );
    ...
    const workspaceRemovePaymentMethodCall = global.fetch.mock.calls.find(
      ([url, options]) =>
        url === "/api/billing/payment-methods/31" && String(options?.method || "").toUpperCase() === "DELETE"
    );
    ...
    expect(urls).toContain("/api/billing/payment-methods");
    expect(urls).toContain("/api/billing/payment-methods/31/default");
    expect(urls).toContain("/api/billing/payment-methods/31/detach");
    expect(urls).toContain("/api/billing/payment-methods/31");
  ...
  it("applies command-correlation headers to correlated write routes and keeps command id stable across csrf retries", async () => {
    ...
    await __testables.request("/api/chat/threads/thread-1/reactions", {
      method: "POST",
      headers: {
        "csrf-token": "provided-token"
      },
    ...
    await __testables.request("/api/admin/workspace/roles", {
      method: "GET"
    });
  ...
  it("applies command-correlation headers to workspace admin write routes", async () => {
    ...
    await __testables.request("/api/admin/workspace/settings", {
      method: "PATCH",
      headers: {
        "csrf-token": "provided-token"
      },
    ...
    await __testables.request("/api/admin/workspace/members/19/role", {
      method: "PATCH",
      headers: {
        "csrf-token": "provided-token"
      },
    ...
    await __testables.request("/api/admin/workspace/invites", {
      method: "POST",
      headers: {
        "csrf-token": "provided-token"
      },
    ...
    await __testables.request("/api/admin/workspace/invites/42", {
      method: "DELETE",
      headers: {
        "csrf-token": "provided-token"
      }
  ...
  it("handles empty-ish wrapper inputs and location/pathname edge cases", async () => {
    ...
    const urls = global.fetch.mock.calls.map(([url]) => url);
    expect(urls).toContain("/api/workspace/invitations/redeem");
    expect(urls).toContain("/api/admin/workspace/members//role");
    expect(urls).toContain("/api/admin/workspace/invites/");
