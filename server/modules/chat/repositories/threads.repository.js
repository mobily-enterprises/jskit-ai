...
function createThreadsRepository(dbClient) {
  ...
  async function repoListForUser(userId, filters = {}, pagination = {}, options = {}) {
    ...
    const activeTombstoneThreshold = toMysqlDateTimeUtc(new Date());
  ...
  async function repoUpdateById(threadId, patch = {}, options = {}) {
    ...
    if (Object.hasOwn(patch, "lastMessageAt")) {
      dbPatch.last_message_at = patch.lastMessageAt ? toMysqlDateTimeUtc(new Date(patch.lastMessageAt)) : null;
    }
...
const repository = createThreadsRepository(db);
